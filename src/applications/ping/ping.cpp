...
PingApp::PingApp(const std::string &multicast_address, uint16_t port,
                 uint8_t ttl, std::function<void(const std::string &)> log,
                 uint64_t instance_id, uint64_t pong_instance_id)
    : ICommandSender<PingApp>(multicast_address, port, ttl),
      EventReceiver<PingApp>(instance_id, multicast_address, port),
      log_(std::move(log)), pong_instance_id_(pong_instance_id) {}
